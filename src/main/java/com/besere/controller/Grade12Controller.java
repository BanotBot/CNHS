...
public class Grade12Controller implements Initializable, StudentTableController {

    @FXML TextField searchField;

    @FXML private TableView<Students> grade12Table;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        grade12Table.setOnMouseClicked((var e) -> {
            if (e.getClickCount() == 2) {
                selectedStud = grade12Table.getSelectionModel().getSelectedItem();
        ...
        grade12Table.setOnKeyPressed(event ->{
            if (event.getCode() == KeyCode.BACK_SPACE || event.getCode() == KeyCode.DELETE) {
                ...
                if (selectedStud != null) {
                    DialogUtil.showConfirmationDelete("DELETE DATA",
                        ...
                        "/images/delete.png",
                        ()->{
                            DeleteDataQuery.deleteRecord("grade12",selectedStud.getId());
    ...
    private void searchListImpl() {
        ...
        searchField.textProperty().addListener((obs,oldval,newval)->{
           ...
           grade12Table.refresh();
        });
        
        grade12Table.setRowFactory(tableview -> new TableRow<Students>(){
            @Override
    ...
    public void loaderDialog() {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/grade12UpdateDialog.fxml"));
        
        try {
            root = loader.load();
            Grade12UpdateController controller = loader.getController();

            controller.setStudentProvider(new StudentProviderImpl(selectedStud));
            controller.setStage(stage);
            controller.setScene(scene);
            controller.setRoot(root);
            controller.setGrade12Controller(this);
            controller.setMainController(maincontroller);
            controller.setDialogStage();

        } catch (IOException ex) {
            Logger.getLogger(Grade12Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    //STYLE DIALOG
    private void setStyleData() {
        for (TableColumn<?, ?> columns : grade12Table.getColumns()) {
            columns.setReorderable(false);
            columns.setResizable(false);
        }
        grade12Table.getStylesheets().add(getClass().getResource("/css/style/grade12Style.css").toExternalForm());
    }
    ...
    private void fetchData() {
        Task<Void> taskFetch = new Task<>(){
            ...
            protected Void call() throws Exception {
                students.clear();
                String sql = "SELECT * FROM grade12";
                try (
        ...
        Platform.runLater(()->{
            grade12Table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY); // set the table to not include the extra col
            grade12Table.setItems(students);
            grade12Table.refresh();
        });
