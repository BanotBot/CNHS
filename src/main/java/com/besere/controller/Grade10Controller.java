...
public class Grade10Controller implements Initializable, StudentTableController {

    @FXML TextField searchField;

    @FXML private TableView<Students> grade10Table;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        grade10Table.setOnMouseClicked((var e) -> {
            if (e.getClickCount() == 2) {
                selectedStud = grade10Table.getSelectionModel().getSelectedItem();
        ...
        grade10Table.setOnKeyPressed(event ->{
            if (event.getCode() == KeyCode.BACK_SPACE || event.getCode() == KeyCode.DELETE) {
                ...
                if (selectedStud != null) {
                    DialogUtil.showConfirmationDelete("DELETE DATA",
                        ...
                        "/images/delete.png",
                        ()->{
                            DeleteDataQuery.deleteRecord("grade10",selectedStud.getId());
    ...
    private void searchListImpl() {
        ...
        searchField.textProperty().addListener((obs,oldval,newval)->{
           ...
           grade10Table.refresh();
        });
        
        grade10Table.setRowFactory(tableview -> new TableRow<Students>(){
            @Override
    ...
    public void loaderDialog() {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/grade10UpdateDialog.fxml"));
        try {
            root = loader.load();
            Grade10UpdateController controller = loader.getController();

            controller.setStudentProvider(new StudentProviderImpl(selectedStud));
            controller.setStage(stage);
            controller.setScene(scene);
            controller.setRoot(root);
            controller.setGrade10Controller(this);
            controller.setMainController(maincontroller);
            controller.setDialogStage();

        } catch (IOException ex) {
            Logger.getLogger(Grade10Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    //STYLE DIALOG
    private void setStyleData() {
        for (TableColumn<?, ?> columns : grade10Table.getColumns()) {
            columns.setReorderable(false);
            columns.setResizable(false);
        }
        grade10Table.getStylesheets().add(getClass().getResource("/css/style/grade10Style.css").toExternalForm());
    }
    ...
    private void fetchData() {
        ...
        Platform.runLater(()->{
            grade10Table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY); // set the table to not include the extra col
            grade10Table.setItems(students);
            grade10Table.refresh();
        });
