...
public class Grade9Controller implements Initializable, StudentTableController {

    @FXML TextField searchField;

    @FXML private TableView<Students> grade9Table;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        grade9Table.setOnMouseClicked((var e) -> {
            if (e.getClickCount() == 2) {
                selectedStud = grade9Table.getSelectionModel().getSelectedItem();
        ...
        grade9Table.setOnKeyPressed(event ->{
            if (event.getCode() == KeyCode.BACK_SPACE || event.getCode() == KeyCode.DELETE) {
                ...
                if (selectedStud != null) {
                    DialogUtil.showConfirmationDelete("DELETE DATA",
                        ...
                        "/images/delete.png",
                        ()->{
                            DeleteDataQuery.deleteRecord("grade9",selectedStud.getId());
    ...
    private void searchListImpl() {
        ...
        grade9Table.setRowFactory(tableview -> new TableRow<Students>(){
            @Override
    ...
    public void loaderDialog() {
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/grade9UpdateDialog.fxml"));
        try {
            root = loader.load();
            Grade9UpdateController controller = loader.getController();

            controller.setStudentProvider(new StudentProviderImpl(selectedStud));
            controller.setStage(stage);
            controller.setScene(scene);
            controller.setRoot(root);
            controller.setGrade9Controller(this);
            controller.setMainController(maincontroller);
            controller.setDialogStage();

        } catch (IOException ex) {
            Logger.getLogger(Grade9Controller.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    //STYLE DIALOG
    private void setStyleData() {
        for (TableColumn<?, ?> columns : grade9Table.getColumns()) {
            columns.setReorderable(false);
            columns.setResizable(false);
        }
        grade9Table.getStylesheets().add(getClass().getResource("/css/style/grade9Style.css").toExternalForm());
    }
    ...
    private void fetchData() {
        ...
        Platform.runLater(()->{
            grade9Table.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY); // set the table to not include the extra col
            grade9Table.setItems(students);
            grade9Table.refresh();
        });
